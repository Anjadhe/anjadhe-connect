// nenva Connect admin — shared shell for every /admin page: the token
...
const RANGE_KEY = 'anjadhe-admin-range';
// The range follows the operator from page to page (same tab), instead of
// snapping back to 30 days on every navigation.
let rangeDays = parseInt(sessionStorage.getItem(RANGE_KEY), 10) || 30;
...
async function api(path, opts = {}) {
  ...
  if (!res.ok) {
    // Surface the server's own sentence when it wrote one (a validation
    // message on a form post), else the bare status.
...
const NAV_ICONS = {
  ...
  website: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><line x1="3" y1="12" x2="21" y2="12"/><path d="M12 3a14 14 0 0 1 0 18a14 14 0 0 1 0-18z"/></svg>',
  ...
  licenses: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="8" r="4"/><path d="M12 11l2 2 7-7"/><path d="M6 12l-2 8 5-3 5 3-2-8"/></svg>',
...
const NAV_GROUPS = [
  { label: 'Monitor', pages: [
    ...
    { id: 'website', label: 'Website', href: '/admin/website' },
...
function buildShell() {
  // One favicon for every admin page, inline so nothing is fetched.
  if (!document.querySelector('link[rel="icon"]')) {
    const icon = document.createElement('link');
    icon.rel = 'icon';
    icon.href = 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><rect width="32" height="32" rx="7" fill="#171717"/><path d="M9 23l7-14 7 14" fill="none" stroke="#fff" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/><circle cx="16" cy="19" r="2" fill="#2f6bff"/></svg>');
  ...
  document.body.insertAdjacentHTML('afterbegin', `
    ...
    <div id="gate" class="hidden">
      <div class="gate-card">
        <div class="gate-word">nenva</div>
        <div class="gate-sub">Connect</div>
        <h1>Admin console</h1>
        <p>Enter the admin token. It stays in this tab only and is forgotten when the tab closes.</p>
        <form class="field" id="gate-form">
          <input id="token-input" type="password" placeholder="Admin token" autocomplete="current-password">
          <button id="token-go" type="submit">Open</button>
        </form>
  ...
  shell.innerHTML = `
    <aside id="side-nav">
      <a class="nav-word" href="/admin"><span class="word">nenva</span><span class="word-sub">Connect Admin</span></a>
  ...
  // Phones: a slim bar that opens the same left nav as a drawer (CSS hides
  // both the bar and the scrim above 640px).
  shell.insertAdjacentHTML('afterbegin', `
    <div id="mobile-bar">
      <button id="nav-open" aria-label="Open navigation" aria-controls="side-nav" aria-expanded="false">${NAV_ICONS.menu}</button>
      <span class="bar-title"><span class="bar-word">nenva</span> ${esc(pageMeta(PAGE).label)}</span>
    </div>
    <div id="nav-scrim"></div>`);
  const dash = $('#dash');
  document.body.insertBefore(shell, dash);
  shell.querySelector('#content').appendChild(dash);
  document.body.insertAdjacentHTML('beforeend', '<div id="tooltip"></div><div id="toast" class="hidden" role="status" aria-live="polite"></div>');
  ...
  $('#admin-nav').addEventListener('click', (e) => { if (e.target.closest('a')) setNav(false); });
...
function navFeedbackBadge(newCount) {
  ...
  if (btn) {
    ...
    if (dot) dot.remove();
    if (newCount > 0) btn.insertAdjacentHTML('beforeend', '<span class="dot"></span>');
  }
}
...
function renderClockNote(day, period) {
  ...
  if (note && day) {
    const days = dayList(day);
    const f = (d) => new Date(d + 'T00:00:00Z').toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' });
    note.innerHTML = rangeDays === 1 ? `Showing <b>${esc(f(day))}</b> (UTC)` : `Showing <b>${esc(f(days[0]))}</b> to <b>${esc(f(day))}</b>, ${rangeDays} UTC days`;
  }
  const el = $('#hdr-clock');
  if (!el) return;
  const now = new Date();
...
// Hover on a pointer, tap on a phone: a tap shows the bar's numbers and a
// tap anywhere else (or a scroll) puts them away.
function wireTooltips(scope) {
  ...
  if (!wireTooltips.wired) {
    wireTooltips.wired = true;
    const hide = (e) => { if (!e.target.closest || !e.target.closest('[data-tip]')) tip.style.display = 'none'; };
...
// ── toast ───────────────────────────────────────────────────────────────
// Confirmations and failures as one quiet line at the bottom, never a
// native alert() that steals focus and blocks the page.
let toastTimer = null;
function toast(message, { error = false, ms = 3200 } = {}) {
  ...
  toastTimer = setTimeout(() => el.classList.add('hidden'), error ? ms * 2 : ms);
}
...
function initAdminShell() {
  ...
  if (range) {
    // Reflect the remembered range, and give the row its label.
    range.querySelectorAll('button[data-days]').forEach(b => b.classList.toggle('on', parseInt(b.dataset.days, 10) === rangeDays));
    if (!range.closest('label')) range.outerHTML = `<label>Days ${range.outerHTML}</label><span class="showing" id="range-note"></span>`;
    $('#range-buttons').addEventListener('click', (e) => {
      const range = e.currentTarget;
      const btn = e.target.closest('button[data-days]');
      if (!btn) return;
      rangeDays = parseInt(btn.dataset.days, 10);
      sessionStorage.setItem(RANGE_KEY, String(rangeDays));
  ...
  $('#gate-form').addEventListener('submit', (e) => { e.preventDefault(); go(); });
