...
// On a phone the nav is one sideways strip; bring the current page into it.
// Runs once the shell is visible — a hidden strip has no widths to scroll by.
function centerActiveNav() {
  const nav = $('#admin-nav');
  const active = nav && nav.querySelector('.is-active');
  if (!active || centerActiveNav.done || !matchMedia('(max-width: 640px)').matches) return;
  centerActiveNav.done = true;
