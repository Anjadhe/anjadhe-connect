...
function evaluate() {
    ...
    if (newsOk + newsFail >= NEWS_FAIL_MIN_TOPICS && newsFail / (newsOk + newsFail) > NEWS_FAIL_THRESHOLD) {
        ...
        alerts.push({
            ...
            detail: `${newsFail} of ${newsOk + newsFail} topic fetches failed (${Math.round(100 * newsFail / (newsOk + newsFail))}%)${kinds ? ' — ' + kinds : ''}. Clients fall back to fetching the feed themselves.`
        });
    }
