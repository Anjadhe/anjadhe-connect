...
const config = {
    ...
    // The commit this process runs. Railway injects RAILWAY_GIT_COMMIT_SHA
    // on every deploy of the public repo; GIT_COMMIT is the manual/CI
    // fallback. Served by /v1/version so anyone can check the running code
    // against github.com/Anjadhe/anjadhe-connect — "the repo is public" and
    // "this is what is running" are two different claims, and this closes
    // the gap between them.
    commit: (process.env.RAILWAY_GIT_COMMIT_SHA || process.env.GIT_COMMIT || '').trim().slice(0, 40) || null,
    ...
    // Service-wide ceiling on new release-notes signups per UTC day. The
    // endpoint is keyless and a static site cannot hold a secret, so per-IP
    // limits are the only per-caller brake; this bounds what a distributed
    // flood can add to the table in a day (0 disables).
    subscribePerDay: intEnv('SUBSCRIBE_PER_DAY', 300),

    // Website analytics (POST /v1/web/events, lib/web.js). The site's own
    // server route (nenva.co/api/e) holds this token and is the only
    // caller; unset = the endpoint answers 503 and nothing is counted. Set
    // the same value as WEB_ANALYTICS_TOKEN on the website's Vercel project.
    webAnalyticsToken: process.env.WEB_ANALYTICS_TOKEN || null,
    // New counter rows per UTC day — the brake on a flood of made-up paths
    // through the site's public /api/e (bumping an existing row is free).
    webMaxRowsPerDay: intEnv('WEB_MAX_ROWS_PER_DAY', 20000),

    // App licenses (lib/license.js). The Ed25519 signing seed, base64 of 32
    // bytes; unset = every /v1/license endpoint answers 503 and nothing is
    // minted. The public half is baked into the app. LICENSE_ALPHA_CLOSES_AT
    // (YYYY-MM-DD) is the paid cutover: after it the self-serve alpha claim
    // refuses and only the admin mint remains. LICENSE_PER_DAY bounds new
    // rows a day, the same aggregate brake the subscriber list has.
    licenseSigningKey: process.env.LICENSE_SIGNING_KEY || null,
    licenseAlphaClosesAt: (process.env.LICENSE_ALPHA_CLOSES_AT || '').trim().slice(0, 10) || null,
    licensePerDay: intEnv('LICENSE_PER_DAY', 200),
    ...
    //     "label": "nenva Cloud", "description": "Fast all-rounder"}}
    ...
    // ── Brokerage linking (/v1/brokerage) via Plaid ─────────────────────
    // The one service that holds a credential to a user's money: a Plaid
    // access token per linked brokerage, sealed at rest with
    // BROKERAGE_TOKEN_KEY (base64 of 32 random bytes, AES-256-GCM) — the
    // secret and the database live in different places, so a copied volume
    // is not a set of live tokens. Holdings and transactions are PROXIED,
    // never stored or logged (the search invariant). Linking is OFF until
    // all three env vars are set; PLAID_ENV picks sandbox or production.
    plaidClientId: process.env.PLAID_CLIENT_ID || null,
    plaidSecret: process.env.PLAID_SECRET || null,
    plaidEnv: process.env.PLAID_ENV === 'production' ? 'production' : 'sandbox',
    plaidClientName: (process.env.PLAID_CLIENT_NAME || 'Anjadhe').slice(0, 30),
    brokerageTokenKey: process.env.BROKERAGE_TOKEN_KEY || null,
    // Linked institutions per install, by tier, and a service-wide ceiling
    // — Plaid's trial plan allows 10 live Items in total, and a paid plan
    // bills per Item, so this is the spend backstop for this service.
    brokerageTierItems: Object.assign({ free: 2, plus: 5, pro: 20 }, jsonEnv('BROKERAGE_TIER_ITEMS', {})),
    brokerageMaxItems: intEnv('BROKERAGE_MAX_ITEMS', 10),
    // Days of bank/card history asked for when a link includes Plaid's
    // Transactions product (Plaid allows 1–730). The first sync of a new
    // link is slower with more; the app shows spending by month, so two
    // years gives year-over-year on day one.
    brokerageHistoryDays: Math.min(730, Math.max(1, intEnv('BROKERAGE_HISTORY_DAYS', 730))),
    // PLAID_MOCK=1 swaps Plaid for canned answers so the link → holdings →
    // transactions → unlink flow runs in dev/tests with no keys.
    plaidMock: process.env.PLAID_MOCK === '1',
    ...
    // Usage counters (search + LLM, per install per calendar month) age
    // out too. They are counts only, but "kept forever" is not a retention
    // statement anyone can rely on. Nothing reads a period this old.
    usageRetentionDays: intEnv('USAGE_RETENTION_DAYS', 400),
