...
const stmt = {
    ...
    brokerageInsertItem: db.prepare(`INSERT INTO brokerage_items
        (item_id, install_id, access_token_enc, institution_id, institution_name, status, last_error, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, 'ok', NULL, ?, ?)`),
    brokerageItems: db.prepare(`SELECT item_id, institution_id, institution_name, status, last_error, created_at, updated_at
    ...
    brokerageInsertLink: db.prepare('INSERT INTO brokerage_links (link_token, install_id, item_id, created_at) VALUES (?, ?, ?, ?)'),
...
module.exports = {
    ...
    brokerageInsertItem: (itemId, installId, tokenEnc, institutionId, institutionName) => {
        const now = new Date().toISOString();
        stmt.brokerageInsertItem.run(itemId, installId, tokenEnc, institutionId || null, institutionName || null, now, now);
    },
    ...
    brokerageInsertLink: (linkToken, installId, itemId) =>
        stmt.brokerageInsertLink.run(linkToken, installId, itemId || null, new Date().toISOString()),
