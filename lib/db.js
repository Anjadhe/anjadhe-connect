...
const stmt = {
    ...
    installList: db.prepare(`SELECT k.install_id, k.tier, k.created_at, k.last_seen_day,
        ...
        ORDER BY (COALESCE(u.used, 0) + COALESCE(l.requests, 0)) DESC, k.created_at DESC LIMIT ?`),
...
module.exports = {
    ...
    installList: (limit = 200) => stmt.installList.all(period(), period(), limit),
    ...
    purgeOldRows: (feedbackDays, analyticsDays) => {
