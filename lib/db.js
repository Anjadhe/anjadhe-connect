...
CREATE TABLE IF NOT EXISTS brokerage_items (
...
CREATE INDEX IF NOT EXISTS brokerage_items_install ON brokerage_items (install_id);
...
CREATE TABLE IF NOT EXISTS web_daily (
    ...
    PRIMARY KEY (day, event, path, referrer, source, country, device)
);
CREATE TABLE IF NOT EXISTS web_uniq (
    day TEXT NOT NULL,
    key TEXT NOT NULL,
    n   INTEGER NOT NULL DEFAULT 0,
    PRIMARY KEY (day, key)
);
CREATE TABLE IF NOT EXISTS web_seen (
    day     TEXT NOT NULL,
    visitor TEXT NOT NULL,
    key     TEXT NOT NULL,
    PRIMARY KEY (day, visitor, key)
);
CREATE TABLE IF NOT EXISTS web_salt (
...
// Linked institutions carry the Plaid products they were linked with
// (2026-09-09: 'investments' for a brokerage, 'transactions' for a bank or
// card, both for an institution that is both) so the app knows what to
// pull; a pending link row carries the same list until the exchange. Rows
// from before the column are brokerages.
for (const t of ['brokerage_items', 'brokerage_links']) {
    if (!db.prepare(`PRAGMA table_info(${t})`).all().some(c => c.name === 'products')) {
        db.exec(`ALTER TABLE ${t} ADD COLUMN products TEXT NOT NULL DEFAULT 'investments'`);
    }
}
    ...
    db.transaction(() => {
        Object.entries(config.llmModels).forEach(([id, v], i) => {
            ...
            ins.run(id, String(meta.upstream),
                String(meta.label || (id === 'anjadhe-cloud' ? 'nenva Cloud' : id)),
...
const stmt = {
    ...
    // The same two, narrowed to a hex PREFIX of the stored id — how the
    // Analytics page finds one install. Without them the grid is a top-N by
    // volume, so a quiet install (or one whose report you are holding) is
    // unreachable no matter how you filter the rows you were given.
    analyticsInstallsPrefix: db.prepare(`SELECT install_id, SUM(n) AS n, COUNT(DISTINCT day) AS days,
            MIN(day) AS first_day, MAX(day) AS last_day
        FROM analytics_daily WHERE day >= ? AND install_id LIKE ? || '%'
        GROUP BY install_id ORDER BY SUM(n) DESC, install_id LIMIT ?`),
    analyticsCellsPrefix: db.prepare(`SELECT install_id, day, SUM(n) AS n FROM analytics_daily
        WHERE day >= ? AND install_id IN (
            SELECT install_id FROM analytics_daily WHERE day >= ? AND install_id LIKE ? || '%'
            GROUP BY install_id ORDER BY SUM(n) DESC, install_id LIMIT ?)
        GROUP BY install_id, day`),
    ...
    metricsTodayPrefix: db.prepare(`SELECT name, n FROM metrics_daily WHERE day = ? AND name LIKE ? || '%' ORDER BY n DESC`),
    ...
    // Brokerage linking (lib/brokerage.js). The sealed Plaid token is the
    // ONLY thing stored per link; holdings and transactions are proxied.
    brokerageInsertItem: db.prepare(`INSERT INTO brokerage_items
        (item_id, install_id, access_token_enc, institution_id, institution_name, products, status, last_error, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, 'ok', NULL, ?, ?)`),
    brokerageItems: db.prepare(`SELECT item_id, institution_id, institution_name, products, status, last_error, created_at, updated_at
        FROM brokerage_items WHERE install_id = ? ORDER BY created_at`),
    brokerageItem: db.prepare('SELECT * FROM brokerage_items WHERE item_id = ? AND install_id = ?'),
    brokerageItemStatus: db.prepare('UPDATE brokerage_items SET status = ?, last_error = ?, updated_at = ? WHERE item_id = ?'),
    brokerageItemToken: db.prepare('UPDATE brokerage_items SET access_token_enc = ?, updated_at = ? WHERE item_id = ?'),
    brokerageDeleteItem: db.prepare('DELETE FROM brokerage_items WHERE item_id = ? AND install_id = ?'),
    brokerageItemCount: db.prepare('SELECT COUNT(*) AS n FROM brokerage_items'),
    brokerageItemCountFor: db.prepare('SELECT COUNT(*) AS n FROM brokerage_items WHERE install_id = ?'),
    brokerageItemsByStatus: db.prepare('SELECT status, COUNT(*) AS n FROM brokerage_items GROUP BY status'),
    brokerageInstallCount: db.prepare('SELECT COUNT(DISTINCT install_id) AS n FROM brokerage_items'),
    brokerageAllProducts: db.prepare('SELECT products, COUNT(*) AS n FROM brokerage_items GROUP BY products'),
    brokerageRenameItems: db.prepare('UPDATE brokerage_items SET install_id = ? WHERE install_id = ?'),
    brokerageInsertLink: db.prepare('INSERT INTO brokerage_links (link_token, install_id, item_id, products, created_at) VALUES (?, ?, ?, ?, ?)'),
    brokerageLink: db.prepare('SELECT * FROM brokerage_links WHERE link_token = ? AND install_id = ?'),
    brokerageDeleteLink: db.prepare('DELETE FROM brokerage_links WHERE link_token = ?'),
    brokeragePurgeLinks: db.prepare('DELETE FROM brokerage_links WHERE created_at < ?'),
    brokerageRenameLinks: db.prepare('UPDATE brokerage_links SET install_id = ? WHERE install_id = ?'),
    ...
    // Website analytics (lib/web.js, POST /v1/web/events).
    webSaltGet: db.prepare('SELECT salt FROM web_salt WHERE day = ?'),
    webSaltSet: db.prepare('INSERT OR IGNORE INTO web_salt (day, salt) VALUES (?, ?)'),
    webSaltDropOld: db.prepare('DELETE FROM web_salt WHERE day <> ?'),
    webSeenDropOld: db.prepare('DELETE FROM web_seen WHERE day < ?'),
    webSeenAdd: db.prepare('INSERT OR IGNORE INTO web_seen (day, visitor, key) VALUES (?, ?, ?)'),
    webUniqBump: db.prepare(`INSERT INTO web_uniq (day, key, n) VALUES (?, ?, 1)
        ON CONFLICT(day, key) DO UPDATE SET n = n + 1`),
    webHasRow: db.prepare(`SELECT 1 AS x FROM web_daily WHERE day = ? AND event = ? AND path = ?
        AND referrer = ? AND source = ? AND country = ? AND device = ?`),
    webRowCount: db.prepare('SELECT COUNT(*) AS n FROM web_daily WHERE day = ?'),
    webBump: db.prepare(`INSERT INTO web_daily (day, event, path, referrer, source, country, device, n, entries)
        VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?)
        ON CONFLICT(day, event, path, referrer, source, country, device)
        DO UPDATE SET n = n + 1, entries = entries + excluded.entries`),
    webUniqSince: db.prepare('SELECT day, key, n FROM web_uniq WHERE day >= ?'),
    webPurgeDaily: db.prepare('DELETE FROM web_daily WHERE day < ?'),
    webPurgeUniq: db.prepare('DELETE FROM web_uniq WHERE day < ?')
};

// Website report columns an admin filter or breakdown may name. The column
// is picked from this list by KEY — request input is never put into SQL.
const WEB_DIMS = ['path', 'referrer', 'source', 'country', 'device'];
const WEB_SUMS = `SUM(CASE WHEN event = 'view' THEN n ELSE 0 END) AS views,
    SUM(entries) AS entries,
    SUM(CASE WHEN event = 'download' THEN n ELSE 0 END) AS downloads,
    SUM(CASE WHEN event = 'signup' THEN n ELSE 0 END) AS signups`;

// Today's salt, made on first use and the only one kept: yesterday's salt
// and yesterday's visitor hashes are deleted the moment a new day's salt
// exists. Kept in the table (not process memory) so a restart mid-day does
// not count every returning visitor twice.
function webSalt() {
    const d = day();
    const row = stmt.webSaltGet.get(d);
    if (row) return row.salt;
    stmt.webSaltSet.run(d, crypto.randomBytes(32).toString('hex'));
    stmt.webSaltDropOld.run(d);
    stmt.webSeenDropOld.run(d);
    return stmt.webSaltGet.get(d).salt;
}
...
module.exports = {
    ...
    migrateInstall: db.transaction((oldId, newId) => {
        ...
        stmt.brokerageRenameItems.run(newId, oldId);
        stmt.brokerageRenameLinks.run(newId, oldId);
    }),

    // Brokerage links. Rows are (hashed install, sealed token, institution
    // name, status) — the institution NAME is the one human-readable
    // value, kept so the admin console can say "3 Fidelity links are
    // broken" without a Plaid call; it names a bank, not a person.
    brokerageInsertItem: (itemId, installId, tokenEnc, institutionId, institutionName, products) => {
        const now = new Date().toISOString();
        stmt.brokerageInsertItem.run(itemId, installId, tokenEnc, institutionId || null, institutionName || null,
            products || 'investments', now, now);
    },
    brokerageItems: (installId) => stmt.brokerageItems.all(installId),
    brokerageItem: (itemId, installId) => stmt.brokerageItem.get(itemId, installId),
    brokerageItemStatus: (itemId, status, lastError) =>
        stmt.brokerageItemStatus.run(status, lastError || null, new Date().toISOString(), itemId),
    brokerageItemToken: (itemId, tokenEnc) => stmt.brokerageItemToken.run(tokenEnc, new Date().toISOString(), itemId),
    brokerageDeleteItem: (itemId, installId) => stmt.brokerageDeleteItem.run(itemId, installId).changes,
    brokerageItemCount: () => stmt.brokerageItemCount.get().n,
    brokerageItemCountFor: (installId) => stmt.brokerageItemCountFor.get(installId).n,
    brokerageInsertLink: (linkToken, installId, itemId, products) =>
        stmt.brokerageInsertLink.run(linkToken, installId, itemId || null, products || 'investments', new Date().toISOString()),
    brokerageLink: (linkToken, installId) => stmt.brokerageLink.get(linkToken, installId),
    brokerageDeleteLink: (linkToken) => stmt.brokerageDeleteLink.run(linkToken),
    brokeragePurgeLinks: (olderThanMs) =>
        stmt.brokeragePurgeLinks.run(new Date(Date.now() - olderThanMs).toISOString()).changes,
    brokerageAllProducts: () => stmt.brokerageAllProducts.all(),
    brokerageStats: () => ({
        items: stmt.brokerageItemCount.get().n,
        installs: stmt.brokerageInstallCount.get().n,
        byStatus: Object.fromEntries(stmt.brokerageItemsByStatus.all().map(r => [r.status, r.n]))
    }),
    ...
    analyticsByInstall: (sinceDay, limit = 100, prefix = '') => {
        const rows = prefix
            ? stmt.analyticsInstallsPrefix.all(sinceDay, prefix, limit)
            : stmt.analyticsInstalls.all(sinceDay, limit);
        ...
        const cells = prefix
            ? stmt.analyticsCellsPrefix.all(sinceDay, sinceDay, prefix, limit)
            : stmt.analyticsCells.all(sinceDay, sinceDay, limit);
    ...
    metricsTodayPrefix: (prefix) => stmt.metricsTodayPrefix.all(day(), prefix),
    ...
    purgeOldRows: (feedbackDays, analyticsDays, usageDays) => {
        ...
        // Website counters share the analytics clock; stale visitor hashes
        // and salts go every sweep regardless (webSalt drops them too, on
        // the first event of a new day — this covers a day with none).
        const analytics = stmt.analyticsPurge.run(daysAgo(analyticsDays)).changes
            + stmt.webPurgeDaily.run(daysAgo(analyticsDays)).changes
            + stmt.webPurgeUniq.run(daysAgo(analyticsDays)).changes;
        stmt.webSeenDropOld.run(day());
        stmt.webSaltDropOld.run(day());
    ...
    // Website analytics (nenva.co). Rows are daily counters over the
    // normalized dimensions in lib/web.js — no IP, no user agent, no URL,
    // no timestamp finer than a day. The one per-visitor thing is web_seen:
    // salted visitor hashes for TODAY only, which is what makes "unique
    // visitors" countable; they go with the salt when the day turns.
    webSalt,
    // One event. `entries` counts a visitor's FIRST event of the day, so a
    // breakdown's entries column is "visitors who arrived this way". Unique
    // counters per key: '*' (anyone), 'p:<path>' (viewed that page),
    // 'e:<event>' (did that). Creating a new counter row stops at
    // maxRowsPerDay — the site's /api/e is public, and a path per request
    // would otherwise grow the table without bound.
    recordWeb: db.transaction((ev, visitor, maxRowsPerDay = 20000) => {
        const d = day();
        const dims = [d, ev.event, ev.path, ev.referrer, ev.source, ev.country, ev.device];
        if (!stmt.webHasRow.get(...dims) && stmt.webRowCount.get(d).n >= maxRowsPerDay) return false;
        const isNew = stmt.webSeenAdd.run(d, visitor, '*').changes > 0;
        if (isNew) stmt.webUniqBump.run(d, '*');
        stmt.webBump.run(...dims, isNew ? 1 : 0);
        const key = ev.event === 'view' ? `p:${ev.path}` : `e:${ev.event}`;
        if (stmt.webSeenAdd.run(d, visitor, key).changes > 0) stmt.webUniqBump.run(d, key);
        return true;
    }),
    webDims: WEB_DIMS,
    // Everything /admin/website draws, for days >= sinceDay. `filters` is
    // {dim: value} over WEB_DIMS and narrows the counter rows; the unique
    // counters (visitors, per-page visitors, the funnel) are service-wide
    // and are returned only unfiltered — a salted hash was never stored
    // per referrer or country, so there is nothing true to narrow.
    webReport: (sinceDay, filters = {}) => {
        const where = ['day >= ?'];
        const args = [sinceDay];
        for (const dim of WEB_DIMS) {
            if (Object.hasOwn(filters, dim)) { where.push(`${dim} = ?`); args.push(String(filters[dim])); }
        }
        const w = where.join(' AND ');
        const daily = db.prepare(`SELECT day, ${WEB_SUMS} FROM web_daily WHERE ${w} GROUP BY day ORDER BY day`).all(...args);
        const totals = db.prepare(`SELECT ${WEB_SUMS} FROM web_daily WHERE ${w}`).get(...args);
        const breakdowns = {};
        for (const dim of WEB_DIMS) {
            breakdowns[dim] = db.prepare(`SELECT ${dim} AS value, ${WEB_SUMS} FROM web_daily WHERE ${w}
                GROUP BY ${dim} ORDER BY entries DESC, views DESC, ${dim} LIMIT 100`).all(...args);
        }
        const filtered = where.length > 1;
        let uniques = null;
        if (!filtered) {
            const byDay = {}, pages = {}, funnel = { visitors: 0, downloadPage: 0, downloaded: 0, signedUp: 0 };
            for (const r of stmt.webUniqSince.all(sinceDay)) {
                if (r.key === '*') { byDay[r.day] = r.n; funnel.visitors += r.n; }
                else if (r.key.startsWith('p:')) pages[r.key.slice(2)] = (pages[r.key.slice(2)] || 0) + r.n;
                else if (r.key === 'e:download') funnel.downloaded += r.n;
                else if (r.key === 'e:signup') funnel.signedUp += r.n;
            }
            funnel.downloadPage = pages['/download'] || 0;
            uniques = { byDay, pages, funnel, today: byDay[day()] || 0 };
        }
        return { daily, totals, breakdowns, uniques };
    },
