...
app.post('/v1/news', auth, async (req, res) => {
    ...
    const out = await Promise.all(topics.map(async (topic) => {
        try {
            const { items, upstream } = await news.topicNews(topic);
            db.bumpMetric('news.topic_ok');
            // How often the second upstream is carrying the service: a
            // steady news.via_bing means Google has stopped answering.
            if (upstream === 'bing') db.bumpMetric('news.via_bing');
            return { topic, items };
        } catch (e) {
...
// ── Brokerage linking (/v1/brokerage, Plaid) ────────────────────────────
// The app's Portfolio links a brokerage here: /link hands back a Plaid
...
function brokerageGate(req, res, next) {
    ...
    if (!allowBrokerageMinute(req.install.install_id)) {
...
function brokerageFail(res, e, what) {
    ...
    if (kind === 'NOT_LINKED') return res.status(404).json({ error: 'No such linked brokerage.', code: 'not_found' });
...
app.post('/v1/brokerage/link', auth, brokerageGate, async (req, res) => {
    ...
    if (!itemId) {
        ...
        if (db.brokerageItemCountFor(req.install.install_id) >= cap) {
            ...
            return res.status(429).json({ error: `Your plan allows ${cap} linked brokerage${cap === 1 ? '' : 's'}. Unlink one to add another.`, code: 'cap', cap });
        }
        if (config.brokerageMaxItems && db.brokerageItemCount() >= config.brokerageMaxItems) {
            db.bumpMetric('brokerage.cap.service');
            return res.status(503).json({ error: 'Brokerage linking is at capacity right now. Try again later.', code: 'capacity' });
        }
    }
    try {
        const r = await brokerage.startLink(req.install.install_id, { itemId });
...
// purpose — one email per release, sent by the operator. Unsubscribing
...
app.post('/v1/subscribe', (req, res) => {
    ...
    if (!origin || !SITE_ORIGINS.has(origin)) {
        ...
        return res.status(403).json({ error: 'Signups are accepted from anjadhe.ai only.', code: 'origin' });
    }
...
app.get('/v1/unsubscribe', (req, res) => {
    ...
    res.type('html').send(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Anjadhe release notes</title>
...
<p><a href="https://anjadhe.ai/changelog">The changelog stays public at anjadhe.ai/changelog.</a></p></body></html>`);
});
...
// ── App licenses ─────────────────────────────────────────────────────────
// The app's one-time license (BUSINESS_MODEL.md in the app repo) is an
// Ed25519-signed string the app verifies OFFLINE with a baked-in public
// key — see lib/license.js. This server only MINTS: the self-serve alpha
// claim below while the alpha is open, and the admin mint (a paid sale by
// hand, until a merchant of record's webhook does it). A license is issued
// to an EMAIL ADDRESS, typed by the person into the app's License card for
// exactly this: re-claiming with the same address returns the same key, so
// the address is how a re-install recovers it. Idempotent by (email, class).
...
function issueLicense({ cls, email, updatesUntil, source, appVersion }) {
    const existing = db.licenseFor(email, cls);
...
// Keyless, nothing about the caller: whether the alpha claim is open. The
// app reads it to decide what the License card offers.
app.get('/v1/license/status', (req, res) => {
    res.json({ enabled: !!_licenseKey, alphaOpen: !!_licenseKey && licenseAlphaOpen(), alphaClosesAt: config.licenseAlphaClosesAt });
});

app.options('/v1/license/alpha', (req, res) => {
    analyticsCors(req, res);
    res.sendStatus(204);
});

app.post('/v1/license/alpha', (req, res) => {
    analyticsCors(req, res);
    if (!_licenseKey) return res.status(503).json({ error: 'Licensing is not enabled on this server.', code: 'disabled' });
    if (!licenseAlphaOpen()) {
        db.bumpMetric('license.closed');
        return res.status(410).json({ error: `The alpha closed on ${config.licenseAlphaClosesAt}; alpha licenses can no longer be claimed.`, code: 'closed' });
    }
    const email = String(req.body?.email || '').trim().toLowerCase();
    if (email.length > 200 || !EMAIL_RX.test(email)) {
        return res.status(400).json({ error: 'That does not look like an email address.', code: 'email' });
    ...
    if (!db.licenseFor(email, 'alpha') && config.licensePerDay && db.licensesToday() >= config.licensePerDay) {
    ...
    const { row, created } = issueLicense({ cls: 'alpha', email, updatesUntil: null, source: 'app', appVersion });
    db.bumpMetric(created ? 'license.alpha' : 'license.alpha_repeat');
    res.json({ success: true, created, ...licenseJson(row) });
});
...
// The operator mints a license by hand: a paid sale before checkout exists,
// or an alpha key for someone who wrote in. `updatesUntil` defaults to a
// year out for paid; alpha is always forever. Returns the key so the
...
app.post('/v1/admin/licenses', adminAuth, (req, res) => {
    ...
    if (!license.CLASSES.has(cls)) return res.status(400).json({ error: 'class must be alpha or paid' });
...
const ADMIN_PAGES = { '': 'overview', overview: 'overview', analytics: 'analytics', installs: 'installs', feedback: 'feedback', subscribers: 'subscribers', licenses: 'licenses', models: 'models' };
