...
Hosted services for the [nenva app](https://nenva.co) (made by Anjadhe LLC), served from
...
exit through Connect's account with one inference provider, DeepInfra
(deepinfra.com), chosen for its zero-data-retention terms; DeepInfra sees
nenva's server, never the user. Upstream error bodies are not forwarded or logged, since they can
echo request text.

Brokerage, bank and card linking (`/v1/brokerage`, via Plaid) is the one service that
holds a credential to a user's money, so it gets the strictest version of
the rule: **holdings, balances and transactions are proxied, never stored
and never logged**. The user signs in on the brokerage's own page (Plaid
Hosted Link in their browser — no brokerage password touches the app or
this server). The only thing stored per link is Plaid's access token,
sealed with `BROKERAGE_TOKEN_KEY` (AES-256-GCM, a secret that lives apart
from the database) alongside the institution's name, so the admin console
can count broken links without a Plaid call. Unlink removes the Item at
Plaid and deletes the row. Plaid's error bodies are never forwarded or
logged (they can echo account data); only the error code travels. The
smoke test's privacy canary covers this path too.
...
Website analytics (`/v1/web/events`) count visits to nenva.co in place of
a third-party analytics script. No cookie and nothing in the visitor's
browser storage; the site skips visitors who send Do Not Track or Global
Privacy Control. The browser posts to the site's own `/api/e`, whose
server route forwards to Connect with `WEB_ANALYTICS_TOKEN`, adding the
visitor's IP, user agent and Vercel's country header. Here the IP and
user agent become one salted hash (the Plausible method) whose salt is
random per UTC day and deleted when the day ends, so a person is one
visitor per day and cannot be followed across days; they are never
written anywhere else (the privacy canary scans for both). Everything
else is normalized against a closed vocabulary (three event names, a
site path, a referring HOST never a URL, a utm_source word, a country
code, a device class) and folded into daily counters.
...
Release-notes subscribers (`/v1/subscribe`) are the second thing a person
types that the service keeps: an email address, given on the website's
download page for exactly one purpose — one email per release, sent by
the operator. The row is the address, a random unsubscribe token, the
signup date and which page it came from. Nothing else: keyless, no
install id, no IP at rest, so it can never be joined to any usage. The
signup response is the same whether or not the address was already on
the list, so the endpoint cannot test membership. Every release email
carries the recipient's own unsubscribe link, and opening it **deletes
the row** — there is no "unsubscribed" state, because the address was
the whole record.

App licenses (`/v1/license/alpha`, minted by hand at `/admin/licenses`)
are the third: an email address typed into the app's Settings › License
card to be issued a license. The row is the address, the class (alpha or
paid), the dates and the key itself — kept for the life of the product,
because it is how a re-install gets the same key back and, for a paid
key, the receipt. No install id, no IP, no Connect key hash on the row, so
it joins to no usage. The key the person holds carries only a hash of the
address, and the app verifies it offline with a public key: the service is
never consulted again, and there is no "phone home" to build a login on.
Removing a row revokes nothing — by design, there is no revocation.
...
clock (`ANALYTICS_RETENTION_DAYS`, 400 days), and so do the per-install
monthly search and LLM usage counters (`USAGE_RETENTION_DAYS`, 400 days).

### Privacy, verified

Two things make the claims above checkable rather than merely stated:

1. **A canary test pins the invariant.** `npm test` (`test/smoke.js`)
   sends a search query and chat prompts whose text is unique to the run
   through every path that handles a body (validation, quota, metering,
   streaming, the error paths), then asserts that the text appears in
   **no console line the server wrote, no response body, and no byte of
   the SQLite file**. A `console.log` of a request body anywhere in the
   service fails CI.
2. **`GET /v1/version` names the running commit.** It returns the git
   commit the process was deployed from (`RAILWAY_GIT_COMMIT_SHA`), which
   you can compare against this repository. "The code is public" and
   "this is the code that is running" are different claims; the endpoint
   closes the gap. `commit` is `null` on an unstamped local run, never a
   made-up value.

What a request carries and what the server keeps, per endpoint, is the
table in the app's `docs/CLOUD_PRIVACY.md`.
...
| `POST /v1/brokerage/link` `{itemId?, products?}` | Start linking an institution through Plaid Hosted Link: returns `{url, linkToken, expiresAt}` — the app opens `url` in the browser. `products` is `["investments"]` (a brokerage, the default) or `["transactions", "investments"]` (a bank or card; the first entry filters Plaid's picker, later ones are taken only where the institution supports them). With `itemId`, re-authorises an existing link (update mode). `429 code: "cap"` at the tier's linked-institution cap, `503 code: "capacity"` at the service ceiling (`BROKERAGE_MAX_ITEMS`), `503 code: "disabled"` when Plaid isn't configured. |
| `GET /v1/brokerage/link/:token` | Poll a link: `{status: "pending" \| "done" \| "exited" \| "expired" \| "unknown", item?}`. On `done` the public token has been exchanged and the Item stored (sealed). |
| `GET /v1/brokerage/items` | This install's linked institutions: `{enabled, items: [{itemId, institution, products, status, createdAt}]}` — `status` is `ok`, `login_required` or `error`; `products` says what the link carries. |
| `POST /v1/brokerage/accounts` `{itemId}` | Proxy of every account on the link with current balances: `{accounts: [{id, name, mask, type, subtype, balance}]}`. |
| `POST /v1/brokerage/transactions/sync` `{itemId, cursor?}` | Proxy of bank/card spending (Plaid Transactions), cursor-based: `{added, modified, removed, accounts, cursor, hasMore}` — no cursor walks the requested history (`BROKERAGE_HISTORY_DAYS`, default 730) in pages; the app keeps `cursor` and calls again while `hasMore`. Each transaction: `{id, accountId, date, name, merchant, amount (positive = money out), pending, pendingId, channel, category: {primary, detailed}}`. |
| `POST /v1/brokerage/holdings` `{itemId}` | Proxy of Plaid Investments holdings, normalised: `{accounts, holdings: [{accountId, security, quantity, costBasis, price, value}]}`. `409 code: "login_required"` when the brokerage wants the user again (re-link with `itemId`). |
| `POST /v1/brokerage/transactions` `{itemId, startDate, endDate?}` | Proxy of investment transactions in the date range, paged server-side: `{accounts, transactions: [{id, accountId, date, type, subtype, quantity, amount, price, fees, security}], total, truncated}`. |
| `POST /v1/brokerage/unlink` `{itemId}` | Remove the Item at Plaid and delete the stored token. |
...
| `POST /v1/web/events` `{event, path, referrer?, source?, country?, device?, ip, ua}` | Website analytics ingest, called only by the site's `/api/e` route (`x-web-token: WEB_ANALYTICS_TOKEN`; unset = `503`). `event` is `view`, `download` or `signup`; crawlers and anything outside the vocabulary answer `{counted: false}`. See Privacy. |
...
| `POST /v1/subscribe` `{email, source?}` | Release-notes list signup from the website (keyless — see Privacy). Requires an `Origin` from the site's own hosts (`SUBSCRIBE_ALLOWED_ORIGINS` adds more); always `{success: true}` for a well-formed address; per-IP hourly limit plus a service-wide `SUBSCRIBE_PER_DAY` cap (default 300, `503 code: "cap"`). |
| `GET /v1/unsubscribe?t=<token>` | The one-click link in every release email. Deletes the address; renders a plain confirmation page. |
| `GET /v1/license/status` | Keyless: `{enabled, alphaOpen, alphaClosesAt}` — whether the self-serve alpha claim below is open. |
| `POST /v1/license/alpha` `{email, appVersion?}` | Claim the free-for-good alpha license (keyless — see Privacy). Mints an Ed25519-signed key the app verifies offline (`lib/license.js`); the same address always gets the same key back, which is how a re-install recovers it. `410 code: "closed"` after `LICENSE_ALPHA_CLOSES_AT`; per-IP hourly limit plus a service-wide `LICENSE_PER_DAY` cap. |
| `GET /v1/admin/licenses` | Every issued license (`{enabled, publicKey, alphaOpen, counts, items}`). |
| `POST /v1/admin/licenses` `{email, class, updatesUntil?}` | Mint by hand: `class` `paid` (updates until the date, default one year out) or `alpha` (forever). Returns the key; sending it is the operator's act. |
| `POST /v1/admin/licenses/remove` `{id}` | Forget one row. Keys verify offline, so this revokes nothing. |
| `GET /v1/admin/subscribers` | The list (`{count, items: [{id, email, token, source, created_at}]}`). |
| `POST /v1/admin/subscribers/remove` `{id}` | Delete one address by hand. |
...
| `GET /v1/admin/web?days=N&path=&referrer=&source=&country=&device=` | Website counters for /admin/website: daily views, arrivals, downloads and signups, a breakdown per dimension, and (unfiltered only) unique visitors, per-page visitors and the funnel. |
| `GET /admin` | Operator pages (browser; enter `ADMIN_TOKEN` in the page). `/admin` is service health; `/admin/analytics`, `/admin/website`, `/admin/installs`, `/admin/feedback`, `/admin/subscribers`, `/admin/licenses` and `/admin/models` are its siblings. |
...
failure rate, news upstream failing, mint spikes, installs hitting quota)
always appear on the dashboard. News counts per TOPIC fetch
(`news.topic_ok`, `news.upstream_fail`, and the failure class
`news.upstream.http429` / `.http403` / `.timeout` / `.net`) because the
route answers per topic, and a request is a 200 even when every topic
inside it failed. Upstream is Google News RSS with Bing's news RSS as the
fallback per topic (`news.via_bing` counts how often it carries the
service); the app deliberately never fetches around a Connect failure
from the user's own address, so this is where resilience lives. Set `ALERT_WEBHOOK_URL` (Slack/Discord webhook or an ntfy.sh
...
3. Set service variables: provider keys, `ADMIN_TOKEN`, optionally
   `PROVIDER_BUDGETS` / quota overrides (see `.env.example`). To issue app
   licenses, `LICENSE_SIGNING_KEY` (base64 of a 32-byte Ed25519 seed —
   `node -e "console.log(require('crypto').randomBytes(32).toString('base64'))"`)
   whose public half the app carries, and `LICENSE_ALPHA_CLOSES_AT` once
   the paid cutover date is decided. To offer brokerage linking,
   `PLAID_CLIENT_ID`, `PLAID_SECRET`, `PLAID_ENV` (`sandbox` until Plaid
   grants production access) and `BROKERAGE_TOKEN_KEY` (base64 of 32
   random bytes, generated the same way — keep it out of the database
   host's backups), plus `BROKERAGE_MAX_ITEMS` sized to your Plaid plan
   (the free trial allows 10 live Items). `BROKERAGE_HISTORY_DAYS` (default
   730) is how much bank/card history a new link asks Plaid for.
...
Then point the nenva app at it with `ANJADHE_CONNECT_URL=https://your.host`.
...
PolyForm Noncommercial 1.0.0 — same as the nenva app. You can read and
