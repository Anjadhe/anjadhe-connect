...
Brokerage linking (`/v1/brokerage`, via Plaid) is the one service that
...
| `POST /v1/brokerage/link` `{itemId?}` | Start linking a brokerage through Plaid Hosted Link: returns `{url, linkToken, expiresAt}` — the app opens `url` in the browser. With `itemId`, re-authorises an existing link (update mode). `429 code: "cap"` at the tier's linked-institution cap, `503 code: "capacity"` at the service ceiling (`BROKERAGE_MAX_ITEMS`), `503 code: "disabled"` when Plaid isn't configured. |
...
| `GET /v1/brokerage/items` | This install's linked institutions: `{enabled, items: [{itemId, institution, status, createdAt}]}` — `status` is `ok`, `login_required` or `error`. |
...
3. Set service variables: provider keys, `ADMIN_TOKEN`, optionally
   ...
   (the free trial allows 10 live Items).
